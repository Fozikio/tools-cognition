...
import type { CortexStore } from '@fozikio/cortex-engine';
...
export const ruminateTool: ToolDefinition = {
  ...
  whenToUse: 'When thinking through a topic over multiple steps to reach a deeper conclusion.',
  inputSchema: {
    type: 'object',
    properties: {
      topic: {
  ...
  async handler(args: Record<string, unknown>, ctx: ToolContext): Promise<Record<string, unknown>> {
    ...
    const depth = typeof args['context_depth'] === 'number' ? args['context_depth'] : 15;
    const shouldExtract = args['extract'] !== false;
